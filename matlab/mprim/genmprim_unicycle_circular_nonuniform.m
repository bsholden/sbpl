function[] = genmprim_unicycle_circular_nonuniform (outfilename)
...
fprintf(1, 'genmprim_unicycle_circular_nonuniform\n');
...
if nargin < 1
    %outputFile = 0;
    basefilename = 'non-uniform_circular_1_';
    outfilename = generate_file_name(basefilename, rmin_m, errmin);
...
ymax = 40;
countmax = 40; 
interm_spacing_m = resolution/4; % Approximate spacing of intermediate poses in meters
interm_spacing_c = interm_spacing_m/resolution;
interm_spacing_rad = 0.02;       % Approximate spacing of intermediate poses in meters
...
%fprintf(fout, 'forwardcostmult: %d\n', forwardcostmult);
%fprintf(fout, 'backwardcostmult: %d\n', backwardcostmult);
%fprintf(fout, 'forwardandturncostmult: %d\n', forwardandturncostmult);
%fprintf(fout, 'backwardandturncostmult: %d\n', backwardandturncostmult);
...
basemprimendpts_c = zeros(3, numberofprimsperangle, 6); % angleind, x_c, y_c, theta_c, costmult, tvoverrv, rv
...
function[] = find_best_prims()
    ...
    for angle_id=1:numberofbaseangles
        ...
        for prim_ind = [3,4,7,8] %4:-1:3
            if (has_backward_prims == 0 && prim_ind > 4) 
                continue;
            end
...
function[outfilename] = generate_file_name(base_name, rmin_m, errmin)
    rmin_s = num2str(rmin_m, 2);
    rmin_s = strrep(rmin_s, '.', 'p');
    err_s = num2str(errmin, 2);
    err_s = strrep(err_s, '.', 'p');
    outfilename1 = strcat(base_name, rmin_s);
    outfilename2 = strcat(outfilename1, '_er');
    outfilename3 = strcat(outfilename2, err_s);
    outfilename = strcat(outfilename3, '.mprim');
end
